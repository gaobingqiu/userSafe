<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!doctype html>
<html>
<head>
<meta charset="utf-8">
<title>账号安全中心</title>
<!-- 移动设备支持 -->
<meta content="text/html; charset=UTF-8" http-equiv="Content-Type">
<meta
	content="width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0, user-scalable=no"
	name="viewport">
<meta content="no-cache,must-revalidate" http-equiv="Cache-Control">
<meta content="no-cache" http-equiv="pragma">
<meta content="0" http-equiv="expires">
<meta content="telephone=no, address=no" name="format-detection">
<meta name="apple-mobile-web-app-capable" content="yes">
<meta name="apple-mobile-web-app-status-bar-style"
	content="black-translucent">
<link href="/css/userSafe/reset.css" rel="stylesheet" type="text/css">
<link href="/css/userSafe/mod40.css" rel="stylesheet" type="text/css">
<link href="/css/userSafe/userSafe.css" rel="stylesheet" type="text/css">
<script src="/js/jquery-1.11.0.min.js"></script>
<script>
	$(function() {
		$('.blankwhite').css('height', '0px');
	});
</script>
</head>
<body>
	<div id="wrap" class="clr">
		<div id="imageswzi">
			<script src="/js/userSafe/swipe2.js" type="text/javascript"></script>
			<div id="displayswipe">
				<div id="banner_box" class="box_swipe">
					<ul id="ul_01">
						<li id="li_01">
							<img src="/images/1.jpg" id="bannerimg31904">
						</li>
					</ul>
					<ol>
						<li class=""></li>&nbsp;
					</ol>
				</div>
			</div>
			<div id="displayone">
				<img src="/images/1.jpg" alt=" " id="onebannerimg">
			</div>
			<script src="/js/userSafe/userSafe_01.js" type="text/javascript"></script>
			<!-- End SlidesJS Required -->
		</div>
		<div class="cont-box clr">
			<ul class="contbg">
				<li class="icon01 li_back" id="li_02">
					<a href="http://www.lanrenmb.com/weiguanwang/" onclick="tongji(650722,0);">
						<p>新罗新闻</p> <img
						src="/images/userSafe/icon01.png"
						style='visibility: hidden;'>
				</a>
				</li>
				<li class="icon02 li_back" id="li_03">
					<a href="http://www.lanrenmb.com" onclick="tongji(650723,0);">
						<p>新罗论坛</p> <img
						src="/images/userSafe/icon02.png"
						style='visibility: hidden;'>
				</a>
				</li>
				<li class="icon03 li_back" id="li_04">
					<a href="javascript:;" onclick="tongji(650724,0);">
						<p>我要爆料</p> <img
						src="/images/userSafe/icon03.png"
						style='visibility: hidden;'>
				</a>
				</li>
				<li class="icon04 li_back" id="li_05">
					<a href="http://www.lanrenym.com" onclick="tongji(650725,0);">
						<p>图说新罗</p> <img
						src="/images/userSafe/icon04.png"
						style='visibility: hidden;'>
				</a>
				</li>
				<li class="icon05 li_back" id="li_06">
					<a href="http://www.lanrenym.com" onclick="tongji(650726,0);">
						<p>龙岩乡讯</p> <img
						src="/images/userSafe/icon05.png"
						style='visibility: hidden;'>
				</a>
				</li>
				<li class="icon06 li_back" id="li_07">
					<a href="http://www.lanrenmb.com" onclick="tongji(650727,0);">
						<p>宣传直达</p> <img
						src="/images/userSafe/icon06.png"
						style='visibility: hidden;'>
				</a>
				</li>
				<li class="icon07 li_back" id="li_08">
					<a href="http://www.lanrenym.com" onclick="tongji(650728,0);">
						<p>聚焦新罗</p> <img
						src="/images/userSafe/icon07.png"
						style='visibility: hidden;'>
				</a>
				</li>
				<li class="icon08 li_back" id="li_09">
					<a href="http://www.lanrenmb.com" onclick="tongji(650729,0);">
						<p>龙津书香</p> <img
						src="/images/userSafe/icon08.png"
						style='visibility: hidden;'>
				</a>
				</li>
				<li class="icon09" li_back><a href="http://www.lanrenym.com"
					onclick="tongji(650730,0);">
						<p>最新活动</p> <img
						src="/images/userSafe/icon09.png">
				</a></li>
			</ul>
		</div>
	</div>
	<script type="text/javascript"
		src="http://res.wx.qq.com/open/js/jweixin-1.0.0.js"></script>
	<script type="text/javascript"
		src="/js/weixin/wxsharejs.js"></script>
	<a href="tel:13600991516">
		<div class="telphone">
			<img src="http://static.dodoca.com/images/tel.png">
		</div>
	</a>
	<script type="text/javascript" src="/js/userSafe/userSafe_02.js"></script>
	<a href="/phone/websiteofficial?uid=87740"> <footer>
			账号安全中心提供服务 </footer>
	</a>
	<div class="blankwhite" style="height: 30px;"></div>
	<script src="/js/userSafe/userSafe_03.js"></script>
	<div id="div_fx_describe" style="display: none;"></div>
</body>
</html>


